...
```javascript:scripts/smart-build.js {15-25}
const fs = require('fs');
...
```javascript:functions/api/[path].js {5-15}
...
```javascript:scripts/build-metrics.js
const fs = require('fs');
...
```javascript:scripts/optimize-assets.js {10-20}
...
```yaml:.github/workflows/deploy.yml {15-25}
...
```javascript:scripts/performance-budget.js
const fs = require('fs');
...
```javascript:functions/metrics.js
export async function onRequest(context) {
